...
Public Sub ExecuteLogin (Email As String, Password As String, Tag As Object) As HttpJob
	Dim ser As B4XSerializator
	Dim data() As Byte = ser.ConvertObjectToBytes(CreateMap("email": Email, "password": Password,  "version": VERSION))
	Return SendJob(CreateJob, data, Tag, "login")
End Sub
...
Public Sub PrintTable (Table As DBResult)
	If Table.Error = Null Then
		Log("Tag: " & Table.Tag & ", Columns: " & Table.Columns.Size & ", Rows: " & Table.Rows.Size & ", Message: " & Table.Message & ", Error: " & Table.Error)
	Else
